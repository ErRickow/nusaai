...
async function run() {
  const result = await nusa.embeddings.embeddings({
...
import { embeddingsEmbeddings } from "nusaai/funcs/embeddingsEmbeddings.js";
...
async function run() {
  const res = await embeddingsEmbeddings(nusa, {
    model: "nusantara-embeddings",
    input: "contoh kalimat",
  });
  if (res.ok) {
