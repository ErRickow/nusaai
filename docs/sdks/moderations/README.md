...
async function run() {
  const result = await nusa.moderations.moderations({
...
import { moderationsModerations } from "nusaai/funcs/moderationsModerations.js";
...
async function run() {
  const res = await moderationsModerations(nusa, {
    input: "Ini adalah teks yang akan dimoderasi",
    model: "text-moderation-latest",
  });
  if (res.ok) {
