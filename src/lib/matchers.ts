...
import { NAIError } from "../models/errors/naierror.js";
...
): MatchFunc<T, E | NAIError | ResponseValidationError> {
  return async function matchFunc(
    response: Response,
    request: Request,
    options?: { resultKey?: string; extraFields?: Record<string, unknown> },
  ): Promise<
    [result: Result<T, E | NAIError | ResponseValidationError>, raw: unknown]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new NAIError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new NAIError("API error occurred", { request, response, body }),
      }, raw];
    }
