...
| ------------ | ---- | ----------- | ----------------------- |
| `apiKeyAuth` | http | HTTP Bearer | `NUSA_XYZ_API_KEY_AUTH` |
...
[`NusaError`](./src/models/errors/nusaerror.ts) is the base class for all HTTP error responses. It has the following properties:
...
```typescript
import { Nusa } from "nusaai";
import * as errors from "nusaai/models/errors";
...
async function run() {
  try {
    ...
    if (error instanceof errors.NusaError) {
...
### Error Classes
**Primary errors:**
* [`NusaError`](./src/models/errors/nusaerror.ts): The base class for HTTP error responses.
...
**Inherit from [`NusaError`](./src/models/errors/nusaerror.ts)**:
...
```typescript
import { Nusa } from "nusaai";
import { HTTPClient } from "nusaai/lib/http";
...
const sdk = new Nusa({ debugLogger: console });
```

You can also enable a default debug logger by setting an environment variable `NUSA_XYZ_DEBUG` to true.
<!-- End Debugging [debug] -->
