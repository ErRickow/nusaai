...
pip install git+ssh://git@github.com/ErRickow/nusaai#speakeasy-sdk-regen-1749741977.git
```

> [!NOTE]
> Once this package is [published to PyPI](https://www.stainless.com/docs/guides/publish), this will become: `pip install --pre neosantaraai`
...
print(chat_completions_response.id)
```
...
Simply import `AsyncNeosantara` instead of `Neosantara` and use `await` with each API call:
...
```sh
# install from the production repo
pip install 'neosantaraai[aiohttp] @ git+ssh://git@github.com/ErRickow/nusaai#speakeasy-sdk-regen-1749741977.git'
```
...
print(chat_completions_response.response_format)
```
...
You can enable logging by setting the environment variable `NEOSANTARA_LOG` to `info`.

```shell
$ export NEOSANTARA_LOG=info
```
...
These methods return an [`APIResponse`](https://github.com/ErRickow/nusaai/tree/speakeasy-sdk-regen-1749741977/src/neosantaraai/_response.py) object.

The async client returns an [`AsyncAPIResponse`](https://github.com/ErRickow/nusaai/tree/speakeasy-sdk-regen-1749741977/src/neosantaraai/_response.py) with the same structure, the only difference being `await`able methods for reading the response content.
...
We are keen for your feedback; please open an [issue](https://www.github.com/ErRickow/nusaai/issues) with questions, bugs, or suggestions.
